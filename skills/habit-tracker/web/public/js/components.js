 ...
 * Note: Uses shared config from config.js (loaded before this file)
 * Fallback definition only if not already available
 */

// Use global HabitCategories from config.js, with fallback
if (typeof window.HabitCategories === 'undefined') {
  window.HabitCategories = {
...
/**
 * Identity Labels Configuration (身份认同)
 * Note: Uses shared config from config.js
 */
if (typeof window.IdentityLabels === 'undefined') {
 ...
 * Note: Uses shared config from config.js
 */
if (typeof window.PresetTags === 'undefined') {
  window.PresetTags = [
 ...
 * Note: Uses shared config from config.js
 */
if (typeof window.ReminderPeriods === 'undefined') {
  window.ReminderPeriods = {
 ...
 * Note: Uses shared config from config.js
 */
if (typeof window.SkipReasons === 'undefined') {
  window.SkipReasons = ['忘了', '太忙', '身体不适', '其他'];
}

/**
 * Repeat Mode Configuration (重复打卡模式)
 * Note: Uses shared config from config.js
 */
if (typeof window.RepeatModes === 'undefined') {
  window.RepeatModes = {
 ...
 * Note: Uses shared config from config.js
 */
if (typeof window.HabitIcons === 'undefined') {
  window.HabitIcons = {
    '早餐': '🌅', '午餐': '☀️', '晚餐': '🌙',
    '站桩': '🧘', '阅读': '📖', '起床': '🌤️', '上床': '🌃',
    '零食': '🍪', '三省吾身': '🤔', '运动': '🏃',
  ...
  return `
    <div class="habit-item ${done ? 'checked' : ''}"
      ...
      <div class="habit-check ${done ? 'checked' : ''}"
           ...
           onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();toggleCheckin(${habit.id})}">
      ...
      <div class="habit-actions">
        <button class="btn btn-xs btn-ghost" onclick="editHabit(${habit.id})" aria-label="编辑习惯 ${escapeHtml(habit.name)}">✏️</button>
        <button class="btn btn-xs btn-ghost" onclick="deleteHabit(${habit.id})" aria-label="删除习惯 ${escapeHtml(habit.name)}">🗑️</button>
      </div>
    </div>
  `;
}
...
function renderCalendarDay(dateStr, day, isToday, isChecked, isOtherMonth) {
  ...
  // Escape dateStr for safe insertion into onclick string
  const escapedDateStr = escapeAttr(dateStr);
  return `
    <div class="${classes}" onclick="showDateDetail('${escapedDateStr}')">${day}</div>
  `;
}
...
function renderMealCard(name, meal) {
  ...
  return `
    <div class="meal-card ${hasRecord ? 'has-record' : ''}" onclick="openMealInput('${name}')">
    ...
    return `
      <div class="record-item log-item" data-id="${record.id}" data-type="log" data-checked-at="${record.checked_at}" data-note="${escapeHtml(record.note || '')}" data-rendered-note="${escapeHtml(renderedNote)}">
        <div class="record-header">
          ...
          <span class="record-time">🕐 ${record.check_time || ''}</span>
          <div class="record-actions">
            <button class="record-btn edit" onclick="editHistoryLog(this)" title="编辑">✏️</button>
            <button class="record-btn delete" onclick="deleteHistoryLog(this)" title="删除">🗑️</button>
          </div>
        </div>
    ...
    return `
      <div class="record-item checkin-item" data-id="${record.id}" data-type="checkin" data-checked-at="${record.checked_at}" data-note="${escapeHtml(record.note || '')}" data-rendered-note="${escapeHtml(renderedNote)}">
        <div class="record-header">
          ...
          <span class="record-time">🕐 ${record.check_time || ''}</span>
          <div class="record-actions">
            <button class="record-btn edit" onclick="editHistoryCheckin(this)" title="编辑">✏️</button>
            <button class="record-btn delete" onclick="deleteHistoryCheckin(this)" title="删除">🗑️</button>
          </div>
        </div>
...
function renderTab(id, icon, label, active = false) {
  return `
    <div class="tab ${active ? 'active' : ''}" data-tab="${id}" onclick="switchTab('${id}')">
      ${icon} ${label}
    </div>
  `;
}
...
function renderCategoryFilter(selected = '') {
  ...
  return `
    <div class="category-filter">
      <button class="category-btn ${!selected ? 'active' : ''}" onclick="filterByCategory('')">全部</button>
...
function renderHeatmap(data) {
  ...
  grid.innerHTML = recentData.map(day => `
    <div class="heatmap-cell level-${day.level}" 
         ...
         onclick="showHeatmapDateDetail('${day.date}')">
    </div>
  `).join('');
}
  ...
  return `
    <div class="stream-item log" data-id="${item.id}" data-checked-at="${createdAt}" data-note="${escapeHtml(item.note || '')}" data-rendered-note="${escapeHtml(renderedNote)}">
      ...
      <div class="stream-actions">
        <button class="stream-action-btn" onclick="editLog(${item.id})" aria-label="修改日志">🕐</button>
        <button class="stream-action-btn" onclick="deleteLog(${item.id})" aria-label="删除日志">🗑️</button>
      </div>
    </div>
  `;
}
...
function renderStreamCheckinItem(item) {
  ...
  return `
    <div class="stream-item checkin" data-id="${item.id}" data-checked-at="${checkedAt}" data-habit-name="${escapeHtml(item.habit_name)}" data-note="${escapeHtml(item.note || '')}" data-rendered-note="${escapeHtml(renderedNote)}">
      ...
      <div class="stream-actions">
        <button class="stream-action-btn" onclick="editCheckinTime(${item.id})" aria-label="修改打卡时间">🕐</button>
        <button class="stream-action-btn" onclick="deleteCheckinFromStream(${item.id})" aria-label="删除打卡">🗑️</button>
      </div>
    </div>
  `;
}
      ...
      <div class="tag-selector">
        ${window.PresetTags.map(tag => `
          <button type="button" class="tag-option ${selectedTags.includes(tag.name) ? 'selected' : ''}" 
                  onclick="toggleTagSelection(this, '${tag.name}')"
      ...
      <div class="identity-selector">
        ${Object.entries(window.IdentityLabels).map(([key, label]) => `
          <div class="identity-option ${selectedKey === key ? 'selected' : ''}" 
               onclick="selectIdentity(this, '${key}')">
      ...
      <div class="reminder-period-grid">
        ${Object.entries(window.ReminderPeriods).map(([key, period]) => `
          <div class="reminder-period-option ${selectedPeriod === key ? 'selected' : ''}"
               onclick="selectReminderPeriod(this, '${key}')">
  ...
  return `
    <div class="skip-reason-container">
      ...
      ${window.SkipReasons.map(reason => `
        <button class="skip-reason-btn" onclick="submitSkipReason(${checkinId}, '${escapeAttr(reason)}')">
          ${escapeHtml(reason)}
        </button>
      `).join('')}
      <button class="btn btn-secondary" style="margin-top: 12px; width: 100%;" onclick="closeModal()">
        取消
      </button>
    </div>
  `;
      ...
      <div class="repeat-mode-selector">
        ${Object.entries(window.RepeatModes).map(([key, mode]) => `
          <div class="repeat-mode-option ${selectedMode === key ? 'selected' : ''}"
               onclick="selectRepeatMode(this, '${key}')">

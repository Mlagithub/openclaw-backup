...
PROJECT_DIR="/home/one/projects/BookReader"
RALPH_LOG_DIR="$PROJECT_DIR/.ralph/logs"
FIX_PLAN="$PROJECT_DIR/.ralph/fix_plan.md"

# 检查 Ralph 进程状态
RALPH_PID=$(pgrep -f "ralph_loop.sh.*BookReader" | head -1)
if [ -n "$RALPH_PID" ]; then
    STATUS="🟢 Ralph 运行中 (PID: $RALPH_PID)"
else
    CLAUDE_PID=$(pgrep -f "claude.*BookReader" | head -1)
...
if [ -f "$LATEST_LOG" ]; then
    RECENT_ACTIVITY=$(tail -100 "$LATEST_LOG" 2>/dev/null | grep -oE 'Phase|Create|Implement|Write|Build|Error|成功|完成' | tail -5)
else
    RECENT_ACTIVITY="无最新日志"
fi

# 检查项目文件结构
FILES_CREATED=$(find "$PROJECT_DIR/app" -type f 2>/dev/null | wc -l)

# 输出汇报
echo "📊 BookReader 项目进展汇报 ($(date '+%Y-%m-%d %H:%M'))"
echo ""
echo "$STATUS"
echo ""
